...
chk('스케줄 목록이 지도를 부른다', () => {
  ...
  /* ☠️ 실제로 재 보니 7자('지도로 동선확인')는 글자 배율 1.44 에서 날짜를 두 줄로 밀고
       건수 배지까지 세로로 쪼갠다. 네 글자까지만 쓴다. */
  const lb = (s.match(/id="calDetailMap">([^<]+)</) || [])[1] || '';
  must(lb.replace(/\s/g, '').length <= 4,
       '버튼 문구가 "' + lb + '" 입니다 — 공백 뺀 4자를 넘기면 큰 배율에서 머리줄이 깨집니다');
  return '버튼 · 목록 · 열기 · "' + lb + '"';
});
...
/* ★ 2026-09-18 — 이 검사는 예전에 '주소 2곳 이상일 때만' 을 못 박고 있었다.
   사용자 요청으로 규칙이 바뀌었다: **갈 곳이 1건이라도 있으면 띄운다.**
   이유는 두 가지다.
     ① 내 위치에서 그 한 곳까지 가는 길이 이제 지도에 그려진다 — 한 곳도 쓸모가 있다.
     ② 주소가 없는 날이야말로 지도가 필요하다. 카드의 [주소 넣기] 로 그 자리에서
        넣을 수 있는데 버튼이 안 보이면 그 길로 갈 수가 없었다.
   ⚠️ 그래도 '아무 날이나' 는 아니다 — 리마인더·고객 카드만 있는 날은 갈 곳이 아니므로
      여전히 안 띄운다. 그 경계를 지키는지가 이 검사의 핵심이다. */
chk('갈 곳이 하나라도 있으면 지도를 권한다', () => {
  const s = read('calendar.js');
  const at = s.indexOf('var _mapBtn');
  must(at > 0, '지도 버튼 조건을 못 찾았습니다');
  const blk = s.slice(at, at + 200);
  must(/_stops\.length >= 1/.test(blk),
       '아직 옛 조건(2곳 이상)입니다 — 한 건짜리 날에 지도가 안 뜹니다: ' + blk.slice(0, 90));
  return '1건 이상';
});

chk('갈 곳이 아닌 것(리마인더·고객)은 지도 대상에서 빠진다', () => {
  /* _mapStops 가 work·shared 만 담기 때문에, 리마인더만 있는 날은 _stops 가 0 이 되어
     위 조건에 안 걸린다. 그 필터가 사라지면 리마인더뿐인 날에도 빈 지도가 권해진다. */
  const s = read('calendar.js');
  const at = s.indexOf('function _mapStops');
  must(at > 0, '_mapStops 를 못 찾았습니다');
  const body = s.slice(at, at + 700);
  must(/it\.type !== 'work' && it\.type !== 'shared'/.test(body),
       '작업·공유일정만 담는 필터가 없습니다 — 리마인더까지 갈 곳으로 셉니다');
  return 'work · shared 만';
});
...
chk('업데이트 안내가 지금 버전에 달려 있다', () => {
  ...
  /* ☠️ 2026-09-21 여기가 **가짜로 통과하고 있었다.**
       예전엔 지금 버전 블록에서 2200자를 잘라 '동선 보기'·'길게 눌러'를 찾았는데,
       그 창이 **다음(예전) 버전 블록까지 넘어가** 거기 적힌 문구를 집었다.
       그래서 새 버전 안내에 그 말이 한 글자도 없어도 통과했다.
       게다가 요구 자체가 틀렸다 — 동선 보기는 3.2.31 에 낸 기능이라,
       그 뒤 모든 버전 안내에 그 말을 넣으라는 건 말이 안 된다.
     → 지금 버전에는 **안내가 있는지**만 보고(위 must), 동선 안내는
       whatsnew 어딘가에 한 번 적혀 있으면 된 것으로 본다. */
  const nxt = wn.indexOf("': {", at + ver.length + 4);
  const blk = wn.slice(at, nxt > at ? nxt - 12 : at + 2200);
  must(/lead:/.test(blk) && /title:/.test(blk), ver + ' 안내가 비어 있습니다');
  must(/\[동선 보기\]/.test(wn) && /길게 눌러/.test(wn),
       '동선 보기·주소 찍기 안내가 whatsnew 에서 통째로 사라졌습니다');
  return 'v' + ver + ' 안내 있음';
});
