...
/* ☠️ 2026-09-08 사용자 지적 — "저장 버튼을 눌렀는데 반응이 없으니 답답해".
     팝업을 줄일 때 기준 ③('화면을 보면 아는 것은 알리지 않는다')을 저장에도 적용했는데,
     폴더 저장은 화면이 하나도 안 바뀐다. 손으로 누른 버튼에는 반드시 답이 있어야 한다.
     자동 저장(auto)·조용한 저장(silent)은 여전히 말이 없어야 한다 — 조건까지 본다. */
chk('손으로 누른 저장은 됐다고 알려 준다', () => {
  const d = fs.readFileSync(path.join(JS, 'dialogs.js'), 'utf8');
  must(/showToast\('저장했습니다', 'ok'\)/.test(d),
       '폴더 저장 성공 신호가 없다 — 버튼을 눌러도 아무 반응이 없다');
  must(/showToast\('이미 저장되어 있습니다', 'ok'\)/.test(d),
       '바뀐 게 없어 건너뛸 때도 답이 있어야 한다');
  const okAt = d.indexOf("showToast('저장했습니다', 'ok')");
  must(/else if \(!isSilent\) \{[\s\S]{0,600}$/.test(d.slice(0, okAt)),
       '저장 토스트가 조용한 저장(silent)까지 뚫고 나온다');
  const at = d.indexOf('변경 없음 - 저장 스킵');
  must(/!isAutoSave && !isSilent/.test(d.slice(at, at + 700)),
       '자동 저장에서도 토스트가 뜬다 — 저장할 때마다 화면을 가린다');
  return '저장 · 이미 저장 (자동·조용한 저장은 그대로 조용)';
});
