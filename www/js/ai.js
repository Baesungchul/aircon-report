...
(function () {
  ...
  function photoMarkGuide(inv) {
    ...
    L.push('- [추천 제목] 블록 안, 본문 제목 줄, 맨 마지막 해시태그 줄에는 마커를 넣지 마세요.');
  ...
  /* ── 추천 제목 3개를 글 맨 위에 (사용자 요청 2026-09-08) ──────────────────
     ...
     ☠️ 왜 필요했나 — 네이버 블로그 앱은 공유로 받은 제목(EXTRA_SUBJECT)을 안 읽는다.
        그래서 제목 칸은 사장님이 손으로 채워야 하는데, 매번 좋은 제목을 지어내는 게 일이다.
        글을 만들 때 같이 지어 두면 고르기만 하면 된다. 따로 부르는 게 아니라 본문과
        한 번에 나오므로 **추가 요금이 없다**.
     ⚠️ 사용자 결정(2026-09-08): 이 블록은 **복사할 때도 그대로 둔다.**
        (물어봤고 "글에 그대로 두기" 를 골랐다 — 블로그 본문에서 직접 지운다)
        나중에 "본문에 안 담기게 해달라" 로 바뀌면, 복사·공유·PC링크 세 곳에서 걷어내면 된다.
     ⚠️ 제목 규칙은 찍고쓰다 ai.js generateTitles 와 같은 원칙이다 — 한쪽만 고치지 말 것.
     ☠️ 순위를 보장한다고 쓰지 말 것. 검색 규칙은 공개돼 있지도, 고정돼 있지도 않다.
        우리가 하는 건 '널리 통하는 원칙'을 따른 후보 셋을 주는 것뿐이고, 고르는 건 사장님이다. */
  var TITLE_BLOCK_GUIDE = [
    ...
    '- 지역명과 업종·작업 키워드를 앞쪽에 둡니다. 뒤로 갈수록 검색 가중치가 낮습니다.',
    '- 사람들이 실제로 검색창에 치는 말로 씁니다.',
    '- 세 개를 서로 다른 각도로: ①지역+업종 키워드 중심 ②본문의 구체적인 사실 중심 ③후기 성격',
  ...
  /* ★ 2026-08-30 — 전 채널(네이버·당근·인스타·페이스북) 글쓰기 공통 지침.
       사용자가 지정한 안전정책·인간다운 문체(AI 티 안 나게)·E-E-A-T·SEO 원칙.
       채널별 sys 뒤에 이어붙인다 — 채널 고유 톤(당근=담백, 인스타=이모지 등)은
       그대로 두고, 이 지침이 그 위에 공통 규칙으로 얹힌다.
       ⚠️ 견적서(quote)·일정 분석(schedule)에는 적용하지 않는다 — 블로그/SNS 글쓰기용 지침이다. */
  var COMMON_WRITE_GUIDE = `[안전 및 정책 고정 지침]
- 성인물, 노출, 음란물, 마약, 도박, 폭력, 살인, 유혈, 잔혹 표현 등 정책 위반 소지가 있는 주제는 글 생성과 이미지 생성 모두 금지합니다.
- 위험하거나 선정적인 방향으로 내용을 유도하지 말고, 관련 요청이 들어오면 생성하지 말고 안전 안내 형태로만 응답합니다.
- 독자를 불안하게 만들거나 자극적인 방식으로 클릭을 유도하는 표현은 사용하지 않습니다.
- 혐오, 자해, 따돌림, 사행성, 유해 요소 미화처럼 정서적 또는 사회적으로 해로운 방향의 내용은 다루지 않습니다.
- 건강, 식품, 영양, 보조 식품 관련 내용은 공인 기관이 공식적으로 인정한 범위를 벗어나 확대 해석하지 않습니다.
- 개인의 체험이나 후기를 모두에게 적용되는 일반적 사실처럼 단정하지 않으며, 치료, 질병 예방, 2차 효과를 암시하는 표현도 피합니다.
- 타인의 프라이버시와 저작권, 초상권, 상표권을 존중하며, 차별적 언어나 모욕적 표현은 허용하지 않습니다.
- 제목, 본문, 이미지 프롬프트 모두 동일한 안전 기준을 적용합니다.

[역할] 당신은 실감 나는 경험과 공인된 데이터를 바탕으로 독자의 공감을 이끌어내고 정보를 명확히 전달하는 노련한 전문 에디터입니다. 단순히 정보를 요약하여 나열하는 기계적 템플릿 형태를 탈피하여, 마치 사람이 직접 연구하고 써 내려간 듯한 '사람 냄새 나는' 따뜻하고 자연스러운 글을 지향합니다.

[AI 문체 및 템플릿 구조 원천 차단]
- 서론: "안녕하세요! 오늘은 ~에 대해 알아보겠습니다", "현대사회에서 ~는 필수적이죠" 같은 뻔하고 진부한 AI식 안부나 정형화된 도입부를 절대 작성하지 마세요. 대신, 독자가 느끼는 고충(Pain Point)을 바로 짚거나 호기심을 자극하는 흥미로운 상황 묘사, 질문으로 글을 시작하십시오.
- 본문: "첫째, 둘째, 셋째" 혹은 "첫 번째로 살펴볼 것은~"처럼 기계적이고 도식적인 구조의 단어 나열을 지양하세요. 소제목 하위의 단락들은 인위적으로 규격화하지 말고, 자연스럽게 인과 관계나 이야기 흐름에 따라 매끄러운 연결 문장을 사용해 연출하십시오.
- 결론: "이상으로 ~에 대한 포스팅을 마치겠습니다", "도움이 되셨다면 공감과 댓글 부탁드립니다" 등의 기계적인 템플릿 아웃트로는 절대 금지합니다. 글에서 언급한 정보를 독자가 일상생활에 어떻게 즉시 적용할 수 있을지에 대한 차분한 조언이나, 깊이 있는 생각할 거리(여운)를 던지며 깔끔하게 글을 맺어주세요.

[인간다운 글쓰기 & 신뢰도(E-E-A-T, DIA+) 향상 지침]
- 실제 경험의 묘사: 독자가 읽으면서 "이 글을 쓴 사람이 실제로 써봤거나 깊게 조사했구나"라고 느낄 수 있게끔 주관적인 시각, 겪어본 사람만이 알 수 있는 세부적인 팁, 장단점 및 솔직한 아쉬운 점을 함께 작성하십시오.
- 객관성과 감정의 균형: 무조건적인 칭찬이나 광고성 극찬("최고의", "대박", "완벽한")을 배제하고, 객관적인 사실과 개인적인 느낀 점을 균형 있게 다루어 정보의 신뢰성(Trustworthiness)을 극대화합니다.
- 구체적인 근거 제시: 모호하고 뭉뚱그려진 표현 대신, 명확한 기준, 대략적인 시간, 구체적인 가격대, 적용 대상 등을 확실하게 명시하여 독자에게 실질적인 가치를 제공하십시오.
- 가독성 높은 문장: 한 문장이 너무 길어지지 않게 호흡을 조절하고, 간결하면서도 리듬감 있는 구어체와 문어체의 적절한 조화(예: 블로그 포스팅 톤앤매너)를 유지합니다.

[검색 친화 원칙(SEO)]
- 제목, 도입부, 본문에 핵심 키워드를 어색하지 않게 자연스러운 문맥으로 어우러지게 배치합니다. (인위적인 키워드 도배 금지)
- 검색 유입 후 체류 시간을 높일 수 있도록, 독자가 이 주제에 대해 추가로 품을 법한 질문(FAQ, 비교 기준, 주의할 점, 실수 패턴 등)을 글의 맥락 속에 매끄럽게 녹여 넣으십시오.

[출력 원칙]
- 메타 설명, AI 가이드라인 등 본문 외의 자기소개나 부가 설명은 일체 배제하고 본문 글만 즉시 출력하십시오.
- 마크다운 구분선(---, ***, ___)을 쓰지 마세요. 블로그 편집기가 선으로 바꿔 주지 않아 본문에 "---" 라는 글자로 그대로 남습니다. 단락은 빈 줄로만 나눕니다.`;

  /* ★ 2026-09-22 개인정보 규칙을 한 곳으로 모은다 ───────────────────────
     ☠️ 사용자 신고: "상가 업체명은 개인정보이니 쓰지 말라고 지침에 넣었는데 반영이 안 된다."
        원인은 지침이 안 들어가서가 아니었다. 지침은 제대로 들어가고 있었다.
        앱이 박아 둔 규칙이 **그 반대를 허락**하고 있었던 것이다 —
          "위치는 아파트/건물명(단지명)까지만 언급합니다"
        상가 현장에서는 건물명이 곧 상호다. 게다가 그 규칙에 '최우선 규칙'이라는
        이름표까지 붙어 있어서, 뒤에 오는 사용자 지침이 이름표 싸움에서 밀렸다.
     → 아파트 단지명은 그대로 둔다(지역 검색어라 빼면 글이 안 읽힌다).
       **상호·점포명만 기본 금지**로 바꾸고, 쓰고 싶은 사람은 지침으로 켠다.
     ⚠️ 이 문장이 채널 4개의 sys 와 기본 지침 4개에 **따로따로 박혀 있었다.**
        그래서 한 군데를 고쳐도 나머지 일곱이 옛말을 했다. 한 곳에서만 고치게 모은다.
     ⚠️ 사진에서 읽지 말라는 말이 한 줄도 없었다. 간판이 찍힌 사진을 같이 보내면서
        상호를 쓰지 말라고만 하면 모델은 사진에서 읽어 쓴다. 그 줄을 넣는다.
     ⚠️ 견적서·일정 분석에는 붙이지 않는다 — 견적서에는 상호가 있어야 한다. */
  var PRIVACY_RULE = [
    '[개인정보 보호]',
    '- 동(棟)·호수, 고객 이름, 전화번호, 상세 주소는 어떤 경우에도 쓰지 않습니다.',
    '- 상호·점포명·업체명(예: ○○치킨, ○○의원)은 쓰지 않습니다. 「○○동 상가」, 「음식점」처럼 일반적으로 적습니다.',
    '- 위치는 지역명(동 단위)과 아파트 단지명까지만 씁니다.',
    '- 사진에 간판·상호·명패·차량번호·서류 글자가 보이더라도 그 내용을 글로 옮겨 적지 않습니다.',
  ...
  var CHANNELS = {
    ...
    daangn: {
      ...
      sys: '당신은 동네 기반 앱 "당근" 비즈프로필의 소식 글을 쓰는 한국어 카피라이터입니다. {업종} 작업 정보와 전/후 사진을 바탕으로 동네 이웃에게 말하듯 친근하고 담백한 소식 글을 작성하세요. 짧고 읽기 쉽게, 과장 광고 표현 없이 실제 작업 내용 위주로 쓰고, 마크다운 기호(#, ** 등)는 쓰지 마세요. 이 글은 {업종} 작업에 대한 글입니다 — 다른 업종의 내용을 끌어오지 마세요.'
    },
    insta: {
      ...
      sys: '당신은 인스타그램 마케팅 캡션을 쓰는 한국어 카피라이터입니다. {업종} 작업 정보와 전/후 사진을 바탕으로 인스타그램 게시글 캡션을 작성하세요. 짧은 문장과 줄바꿈 위주로 리듬감 있게, 이모지를 적절히 쓰고, 마지막에 관련 해시태그를 넣으세요. 마크다운 기호(#제목, ** 등)는 쓰지 마세요(해시태그 제외). 이 글은 {업종} 작업에 대한 글입니다 — 다른 업종의 내용을 끌어오지 마세요.'
    },
    facebook: {
      ...
      sys: '당신은 페이스북 페이지 게시글을 쓰는 한국어 카피라이터입니다. {업종} 작업 정보와 전/후 사진을 바탕으로 신뢰감 있는 페이스북 게시글을 작성하세요. 자연스러운 스토리텔링으로 {단계} 과정과 결과를 전하고, 사실에 근거하되 과장은 피하세요. 마크다운 기호(#, ** 등)는 쓰지 말고 해시태그는 마지막에만 넣으세요. 이 글은 {업종} 작업에 대한 글입니다 — 다른 업종의 내용을 끌어오지 마세요.'
    },
    quote: {
  ...
  async function generatePost(chId, memo) {
    ...
    sys += '\n\n' + PRIVACY_RULE;         // ★ 2026-09-22 한 곳으로 모은 개인정보 규칙
    /* ★ 2026-09-01 사진 배치 규칙은 사용자 지침 **앞**에 둔다.
         사용자가 예전 지침("(사진: 작업 전) (사진: 작업 후) 로 표시")을 저장해 뒀을 수 있는데,
         마커 형식만은 새 규칙이 이겨야 한다 → USER_GUIDE_HEAD 에서 그 하나만 예외로 못 박는다. 마커가 사진으로 바뀌는 곳은 PC 링크(post.html)뿐이라
         네이버 블로그에서만 켠다 — 인스타/당근 캡션에 마커가 남으면 지저분하다. */
    var markGuide = (chId === 'naver') ? photoMarkGuide(photoInventory()) : '';
    /* 추천 제목 블록은 블로그 글에만 붙인다 — 인스타·당근 캡션에 제목 후보가 있으면 지저분하다 */
    if (chId === 'naver') sys += '\n\n' + TITLE_BLOCK_GUIDE;
    if (markGuide) sys += '\n\n' + markGuide;
    ...
    if (memo) ask += '\n\n[추가 메모/강조점]\n' + memo;
    if (chId === 'naver') ask += '\n\n※ 글 맨 위에 [추천 제목] 세 개를 먼저 적고, 빈 줄 하나 뒤부터 본문을 써줘.';
  ...
  function stripMarkdown(t) {
    return String(t || '')
      ...
      .replace(/^\s*[-*+]\s+/gm, '• ')
      .replace(/\[(.*?)\]\((.*?)\)/g, '$1')
      /* ☠️ 2026-09-08 ***, ___ 도 같이 지운다 — --- 만 지우고 있어서 다른 모양의
           구분선이 블로그 본문에 글자로 남았다(사용자 실제 피해). */
