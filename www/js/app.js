...
(function() {
  try {
    // 라이트 딥틸을 기본 테마로. 기존 설치는 1회 마이그레이션(현재 모습 유지)
    if (!localStorage.getItem('ac_theme_migrated_dt')) {
      localStorage.setItem('ac_theme_v1', 'deepteal');
      localStorage.setItem('ac_theme_migrated_dt', '1');
    }
    const theme = localStorage.getItem('ac_theme_v1') || 'deepteal';
    if (theme && theme !== 'dark') {
      document.documentElement.setAttribute('data-theme', theme);
    }
    const fsIdx = parseInt(localStorage.getItem('ac_fs_index_v1') || '2', 10);
    const fsSizes = [13, 14, 15, 17.2, 19.4, 21.6];
    const size = fsSizes[Math.max(0, Math.min(5, fsIdx))] || 15;
    document.documentElement.style.setProperty('--fs-base', size + 'px');
  } catch(e) {}
})();

// 화면 세로 고정 (웹 폴백 — 진짜 고정은 AndroidManifest screenOrientation="portrait")
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ★ 시작 화면: 스케줄(달력) — 스케줄 모달을 index.html에서 처음부터 열어두어 작업탭 깜빡임 없음. 여기선 달력 데이터만 채운다.
  try { if (typeof switchTab === 'function') switchTab('records'); } catch (e) {}
});
...
window.promptPWAInstall = async function() {
  if (!_deferredPWAPrompt) {
    ...
    if (isStandalone) {
      if (typeof showToast === 'function') showToast('이미 앱으로 설치되어 있습니다', 'ok');
    } else {
