...
(function () {
  ...
  AutoBackup.auditFolder = async function () {
    ...
    /* 새 폴더에 .nomedia 를 다시 넣는다 — 이번 실행에서 이미 한 번 시도했더라도 강제로.
       안 하면 옮긴 폴더의 백업 사진이 이번 세션 동안 갤러리에 그대로 보인다. */
    if (sub) { try { await hideBackupFromGallery(true); } catch (e) {} }
    ...
    setTimeout(function () {
      if (sub) {
        alert('백업 폴더가 「' + nm + '」로 지정되어 있어 「' + to + '」로 바꿨습니다.\n\n' +
              ...
              '다른 곳에 두고 싶으면 설정 > 자동백업 > 백업 폴더 바꾸기 에서 언제든 다시 지정할 수 있습니다.');
      } else {
