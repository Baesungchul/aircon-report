...
(function () {
  ...
  function renderDayDetail(dateStr, doScroll) {
    ...
    /* ★ 2026-09-17 그날 지도 — 머리줄 버튼.
       ⚠️ 문구는 '동선 보기'다. 처음엔 '지도'였는데, 그것만으로는 눌러야 뭔지 아는
          버튼이었다(사용자 지적). 이 화면의 쓸모는 지도 자체가 아니라 **그날 도는
          순서**라서(번호 + 점선) 그걸 그대로 적는다.
       ☠️ 네 글자를 넘기지 말 것. 실제로 재 보니 '지도로 동선확인'(7자)은 글자 배율
          1.44 에서 날짜가 두 줄로 밀리고 건수 배지까지 세로로 쪼개진다. */
    /* ★ 2026-09-18 — 조건을 '주소 2곳 이상'에서 '작업 1건 이상'으로 넓혔다(사용자 요청).
       ☠️ 예전 판단은 "한 곳뿐이면 지도를 볼 이유가 없다" 였는데, 두 가지를 빠뜨렸다.
          ① 내 위치에서 그 한 곳까지 가는 길이 이제 지도에 그려진다 — 한 곳이어도 쓸모가 있다.
          ② 주소가 없는 날이야말로 지도가 필요하다. 카드의 [주소 넣기] 로 그 자리에서
             넣을 수 있는데, 버튼이 안 보이니 그 길로 갈 수가 없었다.
       ⚠️ 리마인더·고객 카드만 있는 날은 여전히 안 띄운다 — _mapStops 가 애초에 거른다
          (갈 곳이 아니다). 그래서 'items.length' 가 아니라 '_stops.length' 를 본다. */
    var _stops = _mapStops(items);
    var _mapBtn = (_stops.length >= 1)
      ? '<button type="button" class="cal-detail-map" id="calDetailMap">동선 보기</button>' : '';
