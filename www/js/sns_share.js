...
(function () {
  ...
  var CH = {
    ...
    /* ☠️ 2026-09-08 noShareBody — 본문을 공유로 넘기지 않는다.
         넘기면 네이버가 그 앞부분을 잘라 "[공유] …" 제목을 제 마음대로 만든다
         (사용자 신고, shareTextOnly 주석 참고). 글은 클립보드로 들어간다. */
    naver:  { label: '네이버 블로그', pick: '네이버 블로그', max: 30, gallery: true, noShareBody: true,
              steps: ['<b>1️⃣ 갤러리에 저장</b>을 누르세요 — 사진이 현장매니저 앨범에 담깁니다',
                      ...
                      '<b>제목 칸</b>에는 글의 <b>첫 줄</b>을 넣으세요 (본문에서 잘라 옮기면 됩니다)',
  ...
  async function shareTextOnly(chId, text) {
    ...
    openRefScreen(text);
    try {
      /* ☠️ 2026-09-08 2차 — 제목(title)을 넘겨도 소용이 없었다.
           Capacitor Share 는 title 을 EXTRA_SUBJECT 로 넣는데(플러그인 소스 확인),
           **네이버 블로그 앱이 그 값을 안 읽는다.** 대신 넘겨받은 본문 앞부분을 잘라
           "[공유] " 를 붙여 제목을 만든다. 실제로 이렇게 나왔다:
             "[공유] 삼원빌딩 에어컨 청소 — … 필요가 있습니다. 삼원"
           사용자: "첫줄을 제목으로 넣는게 어려우면 차라리 비워줘"
         → 본문을 공유로 넘기지 않는다. 어차피 안내 3번이 "본문 칸을 길게 눌러
           붙여넣기" 라서, 글은 클립보드로 들어가는 게 원래 경로다. 공유로도 보내면
           같은 글이 두 경로로 들어가면서 제목만 지저분해질 뿐이었다.
         ⚠️ 빈 문자열 대신 공백 한 칸을 보낸다 — 앱에 따라 빈 본문을 아예 안 받는다.
         ⚠️ title 은 그대로 넘긴다. 네이버는 무시하지만 티스토리 등 EXTRA_SUBJECT 를
            읽는 앱에서는 제목이 제대로 들어간다. 손해가 없다. */
      var _ti = firstLine(text);
      var _payload = { text: ch.noShareBody ? ' ' : (text || ''),
